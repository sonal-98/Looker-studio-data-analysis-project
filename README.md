# Looker-studio-data-analysis-project
## IBM HR Attrition Analytics

###  Project Overview

An interactive **HR Attrition Analytics dashboard** created using **Looker/ Data Studio** to analyze employee attrition and identify workforce patterns.

###  Objectives

* Analyze overall employee attrition.
* Compare attrition across departments and job roles.
* Analyze attrition by age, income, gender, and job satisfaction.
* Identify employee groups with higher attrition rates.
* Present HR insights through interactive visualizations.

###  Tools Used

* **Looker** – Dashboard & Data Visualization
* **CSV** – Dataset
* **Calculated Fields** – Advanced analysis
* **GitHub** – Project Documentation

###  Key Insights

* Total Employees: **1,470**
* Employees Who Left: **237**
* Average Monthly Income: **6503**
* Average employee age: **36.9**
* Overall Attrition Rate: **16.1%**
* Highest Department Attrition: **Sales – 20.63%**
* Highest Age-Group Attrition: **18–25 – 36%**
* Highest Income-Bracket Attrition: **Low Income – 28.61%**
* Highest Job-Satisfaction Attrition: **Low Satisfaction – 22.84%**

###  Repository Structure

```text
IBM-HR-Attrition-Looker/
│
├── Dataset/
├── Notebook/
├── Screenshots/
└── README.md
```

##  Dashboard

**Looker Dashboard:**-*https://datastudio.google.com/reporting/8da587cc-63e7-4d5c-91e0-40446f3dc24a*

##  Author

**Sonal Sharma**

*Data Analytics Portfolio Project*
